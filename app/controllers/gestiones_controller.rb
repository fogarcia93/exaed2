class GestionesController < ApplicationController
  ...
  def index
    ...
    if current_user
  @gestiones = current_user.gestiones
  else
  ...
  def create
    ...
    @gestione.user_id = current_user.id
    @gestione.my_file = params[:file]
  ...
  private
    ...
    def gestione_params
      params.require(:gestione).permit(:user_id, :nombre, :correo, :telefono, :departamento, :descripcion, :my_file)
    end
end
